...
<div class="container-fluid pt-4 px-4">
    <div class="bg-secondary rounded p-4">
        <div class="d-flex align-items-center justify-content-between mb-4">
            ...
            <a href="{{ url('admin/passenger-history') }}" class="btn btn-warning"><i
                    class="material-icons opacity-10"></i>Passenger Lists</a>
        </div>
            ...
            <table class="table align-middle table-borderless table-hover mb-0 text-center">
                ...
                <tbody>
                    ...
                    @for ($i = 1; $i <= $numberofPassengers; $i++) @php $seat=explode('|', $ticket->seat);
                        ...
                        <tr>
                            <td>{{ substr($ticket->flight_no, 0, 2) }} {{ substr($ticket->flight_no, 2) }}</td>
                            <td>{{ $ticket->last_name }} , {{ $ticket->first_name }} , {{ $ticket->middle_initial }}
                            </td>
                            <td>₱{{ $ticket->price * $numberofPassengers + $baggage_sum + $total_seat_price }}</td>
                            <td>{{ $ticket->status == '0' ? 'Pending' : ($ticket->status == '1' ? 'Approved' :
                                'Canceled') }}</td>
                            <td>
                                <a href="{{ url('admin/view-details/'.$ticket->id) }}"
